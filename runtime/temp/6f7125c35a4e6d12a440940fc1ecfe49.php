<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:71:"D:\laragon\www\apiicms-dev/application/admin\view\collect\apiiphim.html";i:1721484174;s:66:"D:\laragon\www\apiicms-dev\application\admin\view\public\head.html";i:1721554327;s:66:"D:\laragon\www\apiicms-dev\application\admin\view\public\foot.html";i:1721554327;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="page-container p10">
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">CÀO NGUỒN GỘP</h4>
        </div>
        <div class="card-body">
            <form id="crawl-form">
                <div class="mb-3">
                ...
                <h5>Crawl từ khóa phim</h5>
                <div class="mb-3">
                    <div class="input-group">
                        <span class="input-group-text"><strong>Từ khóa</strong></span>
                        <input id="keyword" type="text" class="form-control">
                        <button id="crawl_by_keyword" type="button" class="btn btn-primary me-1">Tìm kiếm</button>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const buttonCrawlByKeyword = document.getElementById("crawl_by_keyword");
        ...
        buttonCrawlOne.addEventListener('click', () => {
            ...
            fetch("<?php echo url('crawl_apiiphim_link'); ?>", {
                method: "POST",
                headers: {
        ...
        const crawl_page_callback = (currentPage) => {
            const urlPageCrawl = `https://apii.online/apii/danh-sach/phim-moi-cap-nhat?page=${currentPage}`;
            ...
            fetch("<?php echo url('crawl_apiiphim_page'); ?>", {
                method: "POST",
                headers: {
                    'Content-Type': 'application/json'
                },
        ...
        buttonCrawlByKeyword.addEventListener('click', () => {
            const keyword = inputKeyword.value.trim();
            if (!keyword) {
                alert("Mời nhập từ khóa");
                return;
            }
            divMsg.style.display = "block";
            divMsgText.textContent = `Đang tìm kiếm từ khóa: ${keyword}`;

            const urlSearch = `https://apii.online/apii/danh-sach?page=1&search=${keyword}`;
            ...
            fetch(urlSearch, {
              ...
              .then(data => {
                  let results = data.items.map(item => `https://apii.online/apii/phim/${item.slug}|${item._id}|${item.modified.time}|${item.name}|${item.origin_name}|${item.year}`).join("\n");
                  textArealistMovies.value = results;
                  divMsgText.textContent = "Tìm kiếm hoàn tất!";
              });
        });
        ...
        const crawl_movies = () => {
            ...
            fetch("<?php echo url('crawl_apiiphim_movies'); ?>", {
                method: "POST",
                headers: {
                    'Content-Type': 'application/json'
                },
