...
function mac_xml2array($xml)
{
    ...
    $options = LIBXML_NOENT | LIBXML_DTDLOAD | LIBXML_NOCDATA | LIBXML_NOBLANKS;
    $xmlElement = simplexml_load_string($xml, 'SimpleXMLElement', $options);
    if ($xmlElement === false) {
        throw new Exception('Không thể phân tích cú pháp XML');
    }
    $result = json_decode(json_encode($xmlElement), true);
...
function mac_parse_sql($sql='',$limit=0,$prefix=[])
{
    ...
    if ($sql != '') {
        ...
        $sql = str_replace(["\r\n", "\r"], "\n", $sql);
        $sql = explode("\n", trim($sql));
        $cnm = base64_decode('QuG6o24gcXV54buBbiBBcGlpT25saW5l77yMTcOjIG5ndeG7k24gaHR0cHM6Ly9naXRodWIuY29tL2FwaWlwaGltL2FwaWljbXMtZGV2');
        // 循环处理每一行
...
// xử lý tìm kiếm sql - chia sẻ và sửa đổi bởi apii.online
